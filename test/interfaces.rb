...
describe_shared "Persistence::Cell" do
  ...
  describe "in its implementation of the Persistence::Cell interface", self do
    def setup
      @cell = make_cell
    end
...
describe_shared "Persistence::ArrayCell" do
  ...
  describe "in its implementation of the Persistence::ArrayCell interface", self do
    def setup
      @cell = make_cell
    end
...
describe_shared "Persistence::ObjectCell" do
  ...
  describe "in its implementation of the Persistence::ObjectCell interface", self do
    def setup
      @cell = make_cell
    end
...
describe_shared "Persistence::HashRepository" do
  ...
  def make_hash_repo_test_value(data='foo', for_use_with_key=nil)
    data
  end

  describe "in its implementation of the Persistence::HashRepository interface", self do
    def setup
    ...
    it "should set_with_key then get_with_key, returning a different instance" do
      @value = make_hash_repo_test_value('foo', 123)
    ...
    it "should allow store to replace existing" do
      value = make_hash_repo_test_value('foo', 1)
      @repository.set_with_key(1, value)
      assert_equal value, @repository.get_with_key(1)
      value2 = make_hash_repo_test_value('bar', 1)
...
describe_shared "Persistence::IdentityHashRepository" do
  ...
  def make_hash_repo_test_value(data='foo', for_use_with_key=nil)
    values = {:abc => data}
    values[:id] = for_use_with_key if for_use_with_key
    make_entity(values)
  end
  ...
  describe "in its implementation of the Persistence::IdentityHashRepository interface", self do
    def setup
      @repository = make_id_hash_repo
    end
    ...
    it "should store_new then get_by_id" do
      entity = make_entity(:id => 1, :abc => 'foo')
    ...
    it "should reload an identity version, returning the full entity as a different instance" do
      entity = make_entity(:id => 1, :abc => 'foo')
      @repository.store(entity)
    ...
    it "should load an identity version, returning the same instance with the full entity properties" do
      entity = make_entity(:id => 1, :abc => 'foo')
      @repository.store(entity)
      id_entity = make_entity(:id => 1)
    ...
    it "should contains? something stored" do
      entity = make_entity(:id => 1, :abc => 'foo', :def => 'bar')
      @repository.store(entity)
      id_entity = make_entity(:id => 1)
      assert @repository.contains?(id_entity)
    end
    ...
    it "should successfully delete a stored entity" do
      entity = make_entity(:id => 1, :abc => 'foo', :def => 'bar')
      @repository.store(entity)
    ...
    it "should get an array of items in the repo which have a given value for a given property via get_many_by_property, where supported" do
      void_where_unsupported do
        @repository.store(make_entity(:id => 1, :abc => 'a', :def => 'b'))
        @repository.store(make_entity(:id => 2, :abc => 'a', :def => 'a'))
        @repository.store(make_entity(:id => 3, :abc => 'b', :def => 'a'))
        result = @repository.get_many_by_property(:abc, 'a')
        assert_equal [1,2], result.map(&:id).sort
        result = @repository.get_many_by_property(:def, 'a')
